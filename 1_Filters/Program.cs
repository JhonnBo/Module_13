namespace _1_Filters
{    // https://github.com/dotnet/try-samples/blob/main/101-linq-samples/index.md
     // 101 LINQ Samples

    // Основные операции запросов LINQ (C#)
    // https://learn.microsoft.com/ru-ru/dotnet/csharp/programming-guide/concepts/linq/basic-linq-query-operations
    // Введение в запросы LINQ (C#)
    // https://learn.microsoft.com/ru-ru/dotnet/csharp/programming-guide/concepts/linq/introduction-to-linq-queries
    // LINQ to Objects (C#)
    // https://learn.microsoft.com/ru-ru/dotnet/csharp/programming-guide/concepts/linq/linq-to-objects
    class User
    {
    ...
    internal class Program
    {
        ...
        {                
            ...
            foreach (var user in selectedUsers2)
                Console.WriteLine("{0} - {1}", user.Name, user.Age);
            Console.WriteLine('\n' + "То же с использованием методов расширений: ");

            // Выберем пользователей, владеющих английским, которым меньше 28 лет 
            selectedUsers2 = users.Where(u =>u.Age < 28 && u.Languages.Contains("английский"));
            foreach (var user in selectedUsers2)
                Console.WriteLine("{0} - {1}", user.Name, user.Age);
            Console.WriteLine('\n');
        }
